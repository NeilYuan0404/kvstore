...
/* ---------- RESP 编码函数 ---------- */
static int resp_encode_set(char *buf, int idx) {
    char key[32], val[32];
...
static int resp_encode_get(char *buf, int idx) {
    char key[32];
    snprintf(key, sizeof(key), "Teacher%d", idx);
...
static int resp_encode_mod(char *buf, int idx) {
    char key[32], val[32];
    snprintf(key, sizeof(key), "Teacher%d", idx);
...
void benchmark_pipeline(int fd, int count) {
    ...
    int total_cmds = count * 3;   /* SET + GET + MOD */
    ...
    for (int i = 0; i < count; i++)
        pos += resp_encode_set(cmds + pos, i);
    /* GET 阶段 */
    for (int i = 0; i < count; i++)
        pos += resp_encode_get(cmds + pos, i);
    /* MOD 阶段 */
    for (int i = 0; i < count; i++)
        pos += resp_encode_mod(cmds + pos, i);
